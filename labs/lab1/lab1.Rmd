...
1. Read in the OECD PISA data (available at <https://raw.githubusercontent.com/bsouhaib/BA2018/master/data/student_sub.rds>)
...
eigen(B)$values
```
Why are the last two lines the same?
...
- Modify your function to have an optional label:
```{r}
...
\verb|x| is an input to the function that is essential to provide each time and \verb|label| is an optional addition that takes default value \verb|"m"|, but the user could change it to something else, simply by changing it in the function on the fly.
\end{itemize}
